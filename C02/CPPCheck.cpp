...
int main(int argc, char* argv[]){
    ...
    // Find any '.' in the string using the Standard C library function strchr()
    ...
    osarray[guard1] << "#ifndef " << b[base] << "_H" << ends;
    osarray[guard2] << "#define " << b[base] << "_H" << ends;
    osarray[guard3] << "#endif " << b[base] << "_H" << ends;
    ...
    if(!existh){ // Doesn't exist, create it
        ...
        newheader << b[Hline1] << endl << b[guard1] << endl << b[guard2] << endl << b[guard3] << endl;
    }
    ...
    if(existh){ // Already exists; verify it
        strstream hfile; // Write and read
        ...
        if(!strstr(hfile.str(), b[guard1]) ||
           ...
           !strstr(hfile.str(), b[guard3])){
               newheader << b[guard1] << endl << b[guard2] << endl << buf << hfile.rdbuf() << endl
               << b[guard3] << endl << ends;
        }else
        {

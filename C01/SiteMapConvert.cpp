// Using strings to create a custom conversion program that generates HTML output
#include "../require.h"
#include <iostream>
#include <fstream>
#include <string>
...
int main(int argc, char* argv[]){
    requireArgs(argc, 1, "usage: SiteMapConvert inputfilename");
    ifstream in(argv[1]);
    assure(in, argv[1]);
    ...
    while(getline(in, line)){
        if(line.find("<param name=\"source_file\"") != string::npos){
            // Extract data from start of sequence until the terminating quote mark:
            line = line.substr(line.find("value=\"") + string("value=\"").size());
            ...
            while(index < line.size()){
                ...
                string htmlLine = "<a href=\"" + item.path() + "\">" + item.identifier() + "</a><br>";
